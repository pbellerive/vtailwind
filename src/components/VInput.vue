<template>
    <div :class="css.wrapper">
        ...
        <div class="flex">
            <div v-if="withPrepend" :class="[css.prepend, {'bg-blue-fb-300 border-2 border-blue-fb-300 ring-blue-fb-300' : isFocus}]"
                class="flex items-center">
                <span>{{prependText}}</span>
            </div>
            <div class="flex-1">
                <input :type="type" :value="modelValue" :required="required" :class="[css.baseCss]" @input="onInput"
                    :disabled="disabled" @focus="isFocus = true" @focusout="isFocus=false" />
            </div>
            <div v-if="variant == 'danger' && shortErrorMessage" :class="[css.shortError]"
                class="relative grid items-center flex-grow-0">
                <div>
                    {{shortErrorMessage}}
                </div>
