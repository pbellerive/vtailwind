...
export default {
      ...
      css: {
        ...
        variant: {
          default: {
            ...
            optionsWrapper:
              'absolute z-20 border border-gray-300 w-full text-left bg-white text-gray-800',
  ...
  computed: {
    ...
    computedPlaceholder() {
      ...
      if (
        modelType === 'object' &&
        this.modelValue &&
        this.modelValue[this.textOptionAttribute]
      ) {
  ...
  watch: {
    modelValue(_newValue, _oldValue) {
      this.$emit('changed', _newValue);
    },
    query(_newValue, _oldValue) {
      this.filterOptions();
    }
  },
  methods: {
    ...
    isSelectedCss(option) {
      if (
        this.modelValue &&
        this.modelValue[this.valueOptionAttribute] &&
        this.modelValue[this.valueOptionAttribute] === option[this.valueOptionAttribute]
      ) {
    ...
    onClick(option) {
      this.closeOptions();
      const valueToEmit =
        this.valueOptionAttribute === 'object' ? option : option[this.valueOptionAttribute];
