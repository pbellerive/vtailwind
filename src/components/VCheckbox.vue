<template>
  <div>
    <div class="flex align-middle items-center w-fit mb-1" v-if="!!this.$slots['top']">
      <div class="pr-2" v-if="!!this.$slots['tooltip']">
    ...
    <div class="flex">
      <label class="flex" @click="toggle">
        <div class="grid grid-cols-1 place-items-center py-0.5">
          <input type="checkbox" :checked="modelValue" :disabled="disabled" :class="[css.baseCss, css.activeCss, css.focusCss, colorVariantCss]" />
          <font-awesome-icon :icon="['fas', 'check']" aria-hidden="true" :class="[iconCss]" />
        </div>
        <span v-if="!!this.$slots['default']" :class="[css.textCss, textPointerCss]"><slot></slot><span v-if="required" class="text-red-300"> * </span></span>
      </label>
    </div>
    <div v-if="!!this.$slots['bottom']" :class="[css.bottomLabelCss, css.bottomLabelColorCss]">
